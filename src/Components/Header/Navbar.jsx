            ...
            user ?
              <div className='flex items-center gap-3' onMouseEnter={() => setShow(true)} onMouseLeave={() => setShow(false)}>
                <div>
                  ...
                  {
                    show &&
                    <div className=' absolute bg-base-100 shadow border border-gray-400 px-10 py-2 rounded-lg top-13 right-1 z-10'>

                      <div className='mr-3 md:hidden flex justify-center pb-1'>
                        <label className="swap swap-rotate">
                          {/* this hidden checkbox controls the state */}
                          <input onChange={(e) => handleTheme(e.target.checked)} type="checkbox" className="theme-controller" />
                      ...
                      <p className='pb-2 text-gray-500 dark:text-white font-medium'> {user?.displayName} </p>
